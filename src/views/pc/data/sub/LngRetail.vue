...
import { getLngRetail } from '@/api/data'
export default {
  ...
  methods: {
    ...
    async getList() {
      ...
      const res = await getLngRetail(this.listQuery)
      if (res.status === 0) {
    ...
    handleDownload() {
      ...
      const url = `${this.apiUrl}/api/datacenter/price/forecast/export/lngRetail?start=${start}&end=${end}&location=${location}`
      window.open(url)
    }
  }
}

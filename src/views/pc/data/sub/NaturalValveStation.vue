      ...
      fetchJsonp(`http://152.136.232.230:8081/api/screen/price/gatePrice?start=${parseDate(start)}&end=${parseDate(end)}`)
        ...
        .then(json => {
          ...
          for (let index = end.getTime(); index >= start.getTime(); ) {
            ...
            data.push({
              date: date,
              num1: (json.data['北京'] && json.data['北京'][date] && json.data['北京'][date].priceSingle) || defaultNum1,
              num2: (json.data['河北'] && json.data['河北'][date] && json.data['河北'][date].priceSingle) || defaultNum2,
              num3: (json.data['天津'] && json.data['天津'][date] && json.data['天津'][date].priceSingle) || defaultNum3
            })
            index -= 3600 * 1000 * 24
          }

...
import { getCngPrice } from '@/api/data'
export default {
  ...
  computed: {
    tableData() {
      ...
      const res = []
      days.forEach(day => {
        res.push({ date: day, price: this.resData[day].priceSingle })
      })
  ...
  methods: {
    ...
    async getList() {
      ...
      const res = await getCngPrice(this.listQuery)
      if (res.status === 0) {
    ...
    handleDownload() {
      ...
      const url = `${process.env.VUE_APP_BASE_API}/api/datacenter/price/forecast/export/cngPrice?start=${start}&end=${end}&location=${location}`
      window.open(url)
    }
  }
}
